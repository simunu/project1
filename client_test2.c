...
int start_time(long int *time_s);
int end_time(long int *time_e);
int callback(void *arg,int column,char **value,char **name);
sqlite3 *sql_conn(char *db_name);
void sql_create();
void sql_insert(char *chip,char *s,float temp);
void sql_select();
void sql_delect();
...
int main(int argc,char **argv)
{
	...
	while(!g_stop)
	{
		...
		if(!zc)
		{
			printf("zlog get cat failure\n");
			zlog_fini();
			return -1;
		}
		zlog_info(zc,"zlog get cat successfully");
		//socket init
	...
	//	get_time(buff);
		get_temp(&temp,chip);
		snprintf(buf,sizeof(buf),"chip:%s  time:%stemp:%f 'C",chip,s,temp);

		sql_insert(chip,s,temp);
//connect server
		...
		if((info.tcpi_state != TCP_ESTABLISHED))
		{
			...
			while(1)
			{
				...
				{
					zlog_info(zc,"connect to server again");
					break;
				}
			}
		}
...
void sql_create()
{
	...
	sprintf(sql,"create table if not exists data(""chip char(50),"
												"time char(50),"
												"temp char(20));");
...
void sql_insert(char *chip,char *s,float temp)
{
	...
	memset(sql,0,sizeof(sql));
	sprintf(sql,"insert into data values('%s','%s','%f');",chip,s,temp);
	ret=sqlite3_exec(db,sql,NULL,NULL,&error);
	if(ret!=SQLITE_OK)
	{
...
/*  void get_zlog(zlog_category_t *zc)
{
	...
	rc=zlog_init("~/xiongliling/project1/zlog.conf");
	if(rc)
	{
	...
	printf("zlog get cat successfully\n");
}*/
